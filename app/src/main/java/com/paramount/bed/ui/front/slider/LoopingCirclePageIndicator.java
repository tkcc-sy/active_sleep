...
public class LoopingCirclePageIndicator extends LinearLayout {

    private final static int DEFAULT_INDICATOR_WIDTH = 5;
    private ViewPager mViewpager;
    private LoopingPagerAdapter mLoopingPagerAdapter;
    ...
    public LoopingCirclePageIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public LoopingCirclePageIndicator(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public void setViewPager(ViewPager viewPager,LoopingPagerAdapter loopingPagerAdapter) {
        mViewpager = viewPager;
        mLoopingPagerAdapter = loopingPagerAdapter;
    ...
    private final ViewPager.OnPageChangeListener mInternalPageChangeListener = new ViewPager.OnPageChangeListener() {
        ...
        @Override public void onPageSelected(int position) {
            if(mLoopingPagerAdapter.getRealCount() > 0)
                position = position  % mLoopingPagerAdapter.getRealCount();
            if (mViewpager.getAdapter() == null || mLoopingPagerAdapter.getRealCount() <= 0) {
                return;
            }
    ...
    private DataSetObserver mInternalDataSetObserver = new DataSetObserver() {
        @Override public void onChanged() {
            ...
            int newCount = mLoopingPagerAdapter.getRealCount();
            int currentCount = getChildCount();

            if (newCount == currentCount) {  // No change
                return;
            } else if (mLastPosition < newCount) {
                mLastPosition = mViewpager.getCurrentItem() & mLoopingPagerAdapter.getRealCount();
            } else {
                mLastPosition = -1;
            }
    ...
    private void createIndicators() {
        removeAllViews();
        int count = mLoopingPagerAdapter.getRealCount();
        if (count <= 0) {
            return;
        }
